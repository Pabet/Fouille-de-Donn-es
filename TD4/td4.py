...
def naive_bayes_eval(test_sms_file, f):
  ...
  Returns:
    ...
    for line in open(test_sms_file, 'r').readlines(): 
        ...
        #print(cleaned_line)
        spam_index = f(cleaned_line)

        line_list = line.split()
        #print(line_list[0], spam_index)
        if line_list[0] == 'spam':
            spam_in_file += 1.0
            if spam_index == 1:
                recall_counter += 1.0
        ...
        if spam_index == 1:
            identified_as_spam += 1.0
            if line_list[0] == 'spam':
                identified_as_spam_real_spam += 1.0

    #print('recall: ', recall_counter, 'spam in file: ', spam_in_file)
...
#spam_ratio, words, spamicity = naive_bayes_train('test3_1')
#print(naive_bayes_eval('test3_0', lambda sms:'awesome' in sms))  


def classify_spam(sms):
    #Ici, spam_ratio, words, spamicity sont des constantes globales calculées
    # par votre programme (demandez-moi si vous ne comprenez pas!) Vous pouvez
    # supposer la présence du fichier 'SMSSpamCollection' dans le répertoire
    # ou je testerai votre code.
    

    return naive_bayes_predict(spam_ratio, words, spamicity, sms) > 0.5


def naive_bayes_train_better(sms_file):
    """Like naive_bayes_train but with P(ne pas être un spam)"""
    ...
    for i in range(len(words)):
        spamicity.append(ham_frequencies[i]/(spam_frequencies[i]*spam_ratio+ham_frequencies[i]*(1-spam_ratio)))
...
def classify_spam_precision(sms):
    ...
    return naive_bayes_predict(spam_ratio, words, spamicity, sms) < 0.4

#def classify_spam_recall(sms):

split_lines('SMSSpamCollection', 5, 'trainingset', 'testset')
spam_ratio, words, spamicity = naive_bayes_train_better('trainingset')


print(naive_bayes_eval('testset',
    lambda x:naive_bayes_predict(spam_ratio, words, spamicity, x)<0.4))

#classify_spam('hello dear friend buy my amazing stuff')        
